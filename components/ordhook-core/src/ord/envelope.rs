...
use chainhook_sdk::bitcoin::{Script, Transaction};

use super::inscription::Inscription;

use {
    chainhook_sdk::bitcoin::blockdata::{
        opcodes,
        script::{
            self,
...
impl From<RawEnvelope> for ParsedEnvelope {
    fn from(envelope: RawEnvelope) -> Self {
        ...
        // envelope payload 的数量必须是偶数
        // 之前的解析已经跳过了OP_IF和OP_PUSH PROTOCOL_ID
...
#[cfg(test)]
mod tests {
    use chainhook_sdk::bitcoin::{
        self, script::PushBytesBuf, transaction::Version, OutPoint, ScriptBuf, Sequence, TxIn,
        Witness,
    };

    use {super::*, chainhook_sdk::bitcoin::absolute::LockTime};
    ...
    fn pushnum_opcodes_are_parsed_correctly() {
        pub use chainhook_sdk::bitcoin::opcodes;
